...
```{R, het ex1, dev = "svg", echo = F, fig.height = 5.5, message=FALSE, warning=FALSE}
p_load(latex2exp)
set.seed(160322)
n=1e3
ggplot(data = tibble(
  x = runif(n, -2.5, 2.5),
  e = rnorm(n, 0, sd = 4 - 2 * x + 0.3 * x^2)
), aes(x = x, y = e)) +
...
```{R, het ex2 , dev = "svg", echo = F, fig.height = 5.5, message=FALSE, warning=FALSE}
set.seed(160321)
ggplot(data = tibble(
  x = runif(1e3, -3, 3),
...
```{R, het ex3 , dev = "svg", echo = F, fig.height = 5.5, message=FALSE, warning=FALSE}
set.seed(160321)
ggplot(data = tibble(
...
$$
\begin{aligned}
  \hat u_i^2 &= \hat{\alpha}_0 + \hat{\alpha}_1 x_{1i} & \widehat{\text{LM}} &= `r round(lm_bp1, 2)` &\mathit{p}\text{-value} \approx `r round(pchisq(lm_bp1, 1, lower.tail = F), 3)` \\
  \hat u_i^2 &= \hat{\alpha}_0 + \hat{\alpha}_1 x_{1i} \color{#e64173}{+ \hat{\alpha}_2 x^2_{1i}} & \widehat{\text{LM}} &= `r round(lm_bp2, 2)` &\mathit{p}\text{-value} < 0.001
\end{aligned}
$$
---
layout: true
...
$F_{n^\star-k,\,n^\star-k} = \dfrac{\text{RSS}_4}{\text{RSS}_3} \approx\dfrac{`r round(sse_model4, 2) %>% format(big.mark = ",")`}{`r round(sse_model3, 2) %>% format(big.mark = ",")`} \approx`r round(sse_model4 / sse_model3, 2)`$
...
**Step 3:** Calculate White test statistic $\text{LM} = n \times R_u^2 \approx 420 \times `r round(white_r2, 3)`$
...
library(pagedown)
pagedown::chrome_print("16-Heteroskedasticity.html", output = "16-Heteroskedasticity.pdf")
```
