...
1. Load your packages and import the data
```{r, 2-1-1, echo=FALSE, message=FALSE}
...
```
2. Regress earnings in 2009 on years of education. How much do earnings increase (Level & %) on average for every additional year of schooling? (Note: To place a variable into a % interpretation, you need to log transform your data. First start by filling out zero and missing values.)
...
### Q2. Visualizing Residuals (20. pts)
...
Using data of interest, assess the relationship between $X$ and $Y$ for any given pair of variables. What kind of inferences you can draw out from observing the coefficient of interest?

This may be good opportunity to test out your thoughts on how two particular variables may covary.
...
library(pacman)
p_load(tidyverse, ggplot2, ggthemes)

Ohio <- read_csv("C:/Users/phili/Downloads/Ohio.CSV")


Ohio_filt <- Ohio %>% filter(location_name=="Ohio" & sex=="Both")

totals <- aggregate(Ohio_filt$mx, by=list(Ohio_filt$year_id), sum)
colnames(totals) = c("year_id","mx_total")


Ohio_filt <- left_join(Ohio_filt, totals)
Ohio_filt <- Ohio_filt %>% mutate(death_share = 100*(mx/mx_total))

ggplot(Ohio_filt %>% filter(cause_name=="Drug use disorders"))+
  geom_line(aes(x=year_id, y=death_share), color="red") +
  ylab("Share of Deaths - Drug-Related") + xlab("Year")+
  ggtitle("Ohio Death Rate") +
  theme_pander()
